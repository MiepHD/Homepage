class PagesHandler {
    ...
    newPage(category: string, id: number, direction: string): void {
        ...
        new XHR(`pages/${category}/${id}.html`, (result: string) => {
            ...
            `);
            if (id == 1) { this.primarynav.show(); }
            else { this.primarynav.hide(); }
            this.currentid = id;
    ...
    loadPage(destination: number): void {
        const id: number = this.secondarynav.IdToDestination(destination, this.currentid);
    ...
    getCurrentCategory(): string { return this.currentcategory }
}